...
Feature: Per-Operation Upstream Basic Routing
  As an API developer
  I want per-operation upstream URLs to override API-level upstreams
  ...
  Scenario: Per-operation main URL overrides API-level main
    Given I authenticate using basic auth as "admin"
    When I deploy this API configuration:
      """
      apiVersion: gateway.api-platform.wso2.com/v1alpha1
      ...
      spec:
        ...
        operations:
          - method: GET
            ...
            upstream:
              main:
                url: http://sample-backend:9080/op-main
      """
    Then the response should be successful
  ...
  Scenario: Per-operation sandbox-only override proves Bug Fix #3
    Given I authenticate using basic auth as "admin"
    When I deploy this API configuration:
      """
      apiVersion: gateway.api-platform.wso2.com/v1alpha1
      ...
      spec:
        ...
        operations:
          - method: GET
            ...
            upstream:
              sandbox:
                url: http://sample-backend:9080/op-sandbox
      """
    Then the response should be successful
  ...
  Scenario: Per-operation upstream URL update routes to new backend (EDS-stable design)
    Given I authenticate using basic auth as "admin"
    When I deploy this API configuration:
      """
      apiVersion: gateway.api-platform.wso2.com/v1alpha1
